...
# 执行替换操作 - 替换 TLS 设置
sed -i 's/tls: true/tls: false/g' "$config_file"

# 输出替换结果确认
echo "替换完成，当前配置内容："
grep -A 2 -B 2 "\[2602:F7C4:2:000a:0000:0000:5be4:c64c\]:8008" "$config_file" || echo "未找到替换后的服务器地址，请检查配置文件"
grep "tls: false" "$config_file" || echo "未找到替换后的 TLS 设置，请检查配置文件"
